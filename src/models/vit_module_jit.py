...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None):
        # rearrange_heads = Rearrange(
        #     "batch seq_len (num_head h_dim) -> batch num_head seq_len h_dim", num_head=self.num_heads
        # )

        # queries = nn.Sequential(self.query(x), rearrange_heads)
        # keys = nn.Sequential(self.key(x), rearrange_heads)
        # values = nn.Sequential(self.key(x), rearrange_heads)
